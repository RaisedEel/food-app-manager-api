...
/**
 * Allows the creation of a checker for authentication objects. The checker uses the template
 * method pattern to mark the steps of any check. <br/>
 * The checker allow any object of type {@link T} to be compared against the authentication object if and only if an implementation for
 * the object to be converted to {@link User} is provided during the instantiation of the class, other methods can be
 * overridden as needed too.<br/>
...
public abstract class AuthenticationChecker<T> {
  ...
  /**
   * Construct a {@link AuthenticationChecker} with the provided repository.
   ...
   *   <li>Gets the entity with the corresponding id parameter using the {@link #fetchEntity} method and stores it in an {@link Optional}.</li>
   *   <li>If the entity is null returns false.</li>
   *   <li>If the entity is not an {@link User} already, it uses the {@link #convertEntityToUser} method and goes to the next step.</li>
   ...
   * the repository provided in the constructor or if you have a customs find methods in the repository.
   *
  ...
  /**
   * Converts the object to be checked into a {@link User} object. This mapper is needed in order to check
